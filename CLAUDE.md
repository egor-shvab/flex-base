...
| `server/db/field-types/<type>.ts` | its `IFieldSqlModule`: `sql` (`expr` · `filter` · `searchPredicate`, `null` to opt out of search · `sortExpr` only if it orders differently from how it filters · **`filterIndex` and `sortIndex`**, which index kind serves each or `null` for none) and `multi` |
