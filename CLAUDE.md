...
**Migrations are non-destructive.** A new required column is added nullable → backfilled → set `NOT NULL`; `prisma migrate dev` cannot generate that and refuses the diff, so use `--create-only` and edit the SQL. Never drop or retype a column holding user data without an explicit migration plan. **Judge a migration's cost against the table it will eventually run on, not the one in front of you** — a rewrite that is instant on a small table holds an exclusive lock for minutes on a large one, blocking every read and write while it runs.
...
- **Self-documenting names** (`fetchProductDetails`, not `getData`), and **the longer name wins wherever it removes a question** — `readCellValue` over `cellValue`, `applyTableRow` over `setRow`. A name states what a value **is**, not the shape it happens to have: `queryState` holds state, and `queryParams` would be the flat strings it serializes to. Where the code's own prose already names a concept — "a linked record", "a field's configuration" — that phrase is the name. Comment the _why_, not the _what_; use TSDoc for non-obvious business logic.
- **A function name opens with a role prefix, and the role is what it states — never whether the call is async.** `await` marks that at every call site and cannot drift from the signature; a prefix can, and in a layer where nearly every function is async it distinguishes nothing. One exemplar per family, not a list — a new function joins one or says why it cannot:

  | Prefix                         | Contract                                                                                           |
  | ------------------------------ | -------------------------------------------------------------------------------------------------- |
  | `to*`                          | pure conversion X→Y, no I/O — `toSharedRecord`                                                     |
  | `build*`                       | assembles a composite value from parts — `buildRecordWhere`                                        |
  | `parse*`                       | decodes an untrusted string into a shape — `parseRecordQueryState`                                 |
  | `is*` · `matches*` · `should*` | predicate — `isMultiValue`                                                                         |
  | `format*` · `summarise*`       | value → string for display — `formatDate`                                                          |
  | `*For(field)`                  | sync lookup keyed by a field, resolved from a registry (§9) — `inputFor`                           |
  | `require*` · `assert*`         | throws unless a precondition holds — `requireOwnedTable`                                           |
  | `list*` · `get*`               | a service read — `list*` a collection, `get*` exactly one row and throws if absent                 |
  | `fetch*` · `load*` · `ensure*` | a store read — `fetch*` one endpoint, `load*` composes several, `ensure*` only if not already held |

  `get*` retrieves a value that already exists — a row, or a payload as in `getApiErrorMessage` — never one it computes, converts or resolves; that is what keeps it from becoming `getData`. And the two sync members of otherwise-async service objects, `buildOptions` and `collectRelationTargets`, read as sync from their prefix alone: the property this rule exists to keep.
...
- **A composable belonging to exactly one component lives in that component's own folder**, not in `app/composables/` — `Foo.vue` becomes `Foo/Foo.vue` beside `Foo/useThing.ts`, its specs and any rig they share. `app/composables/` is for what the whole app may reach for; scope is stated by location, not by a warning in a doc comment. `BaseSelect/` is the one today. **A page is the exception**, because Nuxt file routing would read a sibling folder as a route: a composable owned by one page stays in `app/composables/` and is scoped by its TSDoc alone (`useRecordListQuery`, `useRecordDetail`). Auto-import is unaffected (`pathPrefix: false` names by filename), and `nuxt.config.ts` pins `extensions: ['.vue']` so a `.ts` sitting there is **not** registered as a component.
...
- Forms use the `useForm` composable — reactive fields, per-field zod errors that clear on edit, form-level server error, `pending`, `submit`.
...
**Indexability is declared, not inferred.** A type says which index kind serves its filter and its ordering — the kind follows how it _compares_, so an unanchored `ILIKE` wants trigrams where a range wants a B-tree, and a projection nothing local can cover (RELATION orders by a value in another row) says `null`. Nothing is indexed until a field opts in (`Field.indexed`); `server/db/field-indexes.ts` turns the declarations into DDL. See `docs/architecture.md` §9 for the four rules that are not guessable from the code.
...
**Inputs, filters and config summaries are data, not components** — an input or filter is an `IFieldControl` naming a `Base*` control, a `props(field)` factory, and the adapters between that control's model and the field's value; a config summary is `(field, ctx) => string`, its context supplied by the caller. Only cells are components. A type whose control needs data beyond its own metadata gets that component in its own folder (RELATION's picker is the only one today).
