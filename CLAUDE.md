...
- **Reuse before you paste.** A declaration block that would be the second copy of an existing one belongs in `_mixins.scss` instead. Check the mixin list there first — `focus-ring`, `stack`, `field-label`, `field-error`, `form-control`, `error-banner`, `page-header`, `page-title`, `page-empty`, `back-link`, `text-link` already cover most of what a new component or page needs.
...
- **Current State:**
  ...
  - **app/assets/scss/** — `main.scss` entry `@use`s `_variables.scss`, `_reset.scss`, `_auth-form.scss`. `_variables.scss` builds the token surface from `_palette.scss` (primitives as SCSS variables) and `@use`s both `palette` and `functions`; the `*-tint` alpha washes are spelled out because a custom property's alpha cannot be modified in CSS. `_functions.scss` (`rem()`) **and `_mixins.scss`** are both auto-injected into SFC styles via Vite `additionalData`; a standalone partial gets neither, so `_auth-form.scss` `@use`s both itself (and `_mixins.scss` `@use`s `functions`, since it does not re-export `rem()`). Mixins: `focus-ring($offset)`, `stack($gap)`, `field-label`, `field-error`, `form-control`, `error-banner`, `page-header`, `page-title`, `page-empty`, `back-link`, `text-link` — each replaced 2–6 copy-pasted blocks
  - **Design direction: "Workspace"** (chosen after a six-concept exploration) — a deliberately plain, familiar office-app look for a largely non-technical audience: 16px base, 44px minimum target, 8px control radius, a single accent blue (`#1C64D8`), two shadows, borders doing the structural work. **Phase 1 of 5 is complete**: the token layer, the focus ring, 44px filled buttons and form fields, and the 16px control type scale. Still to come — the app shell with a persistent table sidebar (2), the records page incl. 52px rows and the row `⋮` menu (3), table-scoped search (4), and a sweep of the remaining pages (5). **The 44px minimum is therefore not yet met by icon-only buttons**; that lands in phase 3 with the surfaces they size
