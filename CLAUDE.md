...
- **Reuse before you paste.** A declaration block that would be a second copy belongs in `_mixins.scss`. Check the list there first — `focus-ring`, `below-shell`, `stack`, `field-label`, `field-error`, `form-control`, `error-banner`, `page-header`, `page-title`, `page-empty`, `text-link` cover most needs.
