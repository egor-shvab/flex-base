...
5. One entry in `FILTER_VALUE_BY_TYPE` (`shared/constants/filter.ts`) — its filter value's `shape` (`scalar`/`range`, which also names the params it claims) and `empty` value, alongside its shape in `IFilterValueByType` (`shared/types/filter.ts`) — plus one in `FIELD_SQL_BY_TYPE` (`server/services/record-query.ts`) giving its SQL projection, how that projection is compared, and **how free-text search matches it** (`searchExpr`, returning `null` to opt out). A type that reads as something other than what it stores also declares an optional `sortExpr` there (RELATION is the only one: it filters on the stored id but orders by the target's label). `searchExpr` is separate from `expr` because NUMBER and BOOLEAN cast, and neither `numeric` nor `boolean` has an `ILIKE` operator — NUMBER searches the un-cast text, BOOLEAN opts out (searching `e` would match every `false`), and RELATION opts out (its stored value is a cuid; matching the label instead would mean `targetLabel`'s correlated subquery against every row, since the count query has no `LIMIT`).
...
- **Current State:**
  ...
  - **Design direction: "Workspace"** (chosen after a six-concept exploration) — a deliberately plain, familiar office-app look for a largely non-technical audience: 16px base, 44px minimum target, 8px control radius, a single accent blue (`#1C64D8`), two shadows, borders doing the structural work. **All five phases are complete**: the token layer and focus ring, the app shell (header + persistent table sidebar + breadcrumbs), the records page (52px rows, the filter summary line, worded pagination, friendlier cell copy), table-scoped search, and the closing sweep. **The 44px minimum target is met everywhere**, icon-only buttons included. The header deliberately ships **without** a global "Search everything" box: cross-table search is not built, and a dead input is worse than a gap
  - **`app/error.vue`** — the whole-app error boundary, added in phase 5 to fix a real defect: both inner pages forwarded the upstream `statusCode` but hard-coded `statusMessage: 'Table not found'`, so a malformed `?search=`/`?sort=` returned 400, failed Nuxt's `is404` check and rendered the **500** template claiming a table that had just loaded did not exist. `toPageError` (`app/utils/api-error.ts`) now asserts a cause only for a 404. The page is deliberately **store-free** — it has to render when data fetching is exactly what failed
  - **A failed refetch is visible, not silent.** `records.ts` gained `failed`, set in a `catch` that rethrows; the records page's `watch` swallows the rejection (an unhandled one in a watcher left the table showing rows that no longer matched the URL) and shows a banner. The empty state is suppressed while `failed`, because an empty result and an unknown result are indistinguishable in the store and "No records yet" would be a guess
  - **Focus: two defects fixed, the trap still owed.** The off-canvas sidebar is `visibility: hidden` as well as translated — translating alone left it off-screen but _focusable_, so mobile keyboard users tabbed into an invisible menu on every page. `BaseModal` marks `#__nuxt` `inert` while open, which makes its `aria-modal="true"` true rather than a claim, and as a side effect keeps Tab inside the dialog. **Still missing: moving focus into a dialog on open and restoring it on close.** Known gap, not an oversight — the edge cases (nested modals teleporting out of the sidebar, the sidebar auto-closing on route change, breakpoint crossing mid-animation, honouring existing `autofocus`) are a piece of work in their own right
  - **`--color-border-control` is separate from `--color-border-strong` on purpose.** A divider only has to be visible; a control's outline is the only thing identifying the control, so it carries the 3:1 non-text-contrast floor. Neither `$gray-200` (1.3:1) nor `$gray-300` (1.66:1) clears it — `$gray-400` (3.17:1) exists for exactly this, and `BaseButton --secondary` is surface-on-surface so its border is load-bearing
  ...
  - **app/components/common/**
    ...
    - `BaseButton.vue` — button atom (type/disabled props, slot content); `primary` (default) / `secondary` / `danger` / `icon` / `ghost` / `link` variants, applied as one `base-button--${variant}` class. `secondary` is the neutral peer of `primary` — same geometry so a dialog's footer pair aligns, bordered rather than filled; it is what `ConfirmModal`'s Cancel uses, since two filled buttons made the _safe_ action the heaviest thing on a delete dialog. A typed `tone?: 'default' | 'danger'` replaced a free-form `hoverColor` string: all four call sites passed the same danger token, so a closed set says the same thing without letting an arbitrary colour into the system. **`.base-button` is a neutral chassis** (flex centering, radius, type, `focus-ring`, `:disabled`) and the filled look lives in `&--primary` — which costs no template change, because the template always emits a variant class. `primary`/`danger`/`ghost` are `min-height: var(--control-height)`; `icon` and `link` stay small on purpose (they set row heights across the app — see §4). `font-size`/`font-weight` must stay on the chassis: `--ghost` declares neither. `icon` is a borderless icon-only button taking `icon` (iconify name) + `label` (aria-label/title); `link` is a bare text button for row actions (the chrome of a link, the semantics of a button) used for Rename/Edit/Delete on the dashboard and field manager. Both read a `--hover-color` custom property that each variant defaults for itself (`icon` → text, `link` → accent); the optional `hoverColor` prop overrides it via an inline style, which is how the Delete actions and the choice-remove button go danger-red — all four call sites pass the same `var(--color-danger)`, so this wants to become a typed `tone` prop. Per-variant defaults are why this is a custom property rather than the `v-bind` it used to be. The `ghost` variant is a transparent text+icon button with a faint accent hover background (`--color-accent-tint`). Every variant has `:hover`, `:active` and a keyboard focus ring. First `@nuxt/icon` `<Icon>` usage in the app.
  ...
  - **Search wire format:** one reserved `?search=` param, ANDed with the filters. It is **free text ORed across the table's searchable columns**, which is the only OR anywhere in the query layer — hence the parenthesised group in `buildRecordSearch`; see the note under `record-query.ts` below. `SEARCH_MIN_LENGTH` (2) is enforced by the **query schema**, not just the input, so no caller can trigger an unanchored full-table scan with one character — a shorter term is a 400, and the client simply drops it rather than sending it. `?search=` empty reads as absent, exactly like a filter
