...
This is a full-stack Nuxt 4 application using the `app/` source directory convention (Nuxt 4 default), with the Nitro server layer under `server/`, shared client/server code under `shared/`, and the database layer under `prisma/`. Implemented so far: the full Prisma schema, authentication (register/login/logout, session restore, route guards), table metadata CRUD (dashboard + ownership-scoped API), field metadata CRUD (typed fields per table with per-type options validation), record CRUD (paginated API, metadata-driven validation, field-type component registry, `DynamicForm`/`DynamicTable`), and URL-driven filtering & sorting (a filter drawer generated from the field metadata — one control per field, all conditions ANDed — plus click-to-sort headers over a raw-SQL record query), relations between tables (a single-link RELATION field with a server-resolved display label, its own picker, filter and label-ordered sort), and a per-table record number shown as the leading `Record #` column. **The MVP scope is complete**; what remains is a test suite.
...
**In a query the number behaves like a field**, via one seam rather than a special case at each
layer. `RECORD_NUMBER_FIELD` (`shared/utils/filter.ts`) is an ordinary `IField` of type `TEXT`
under the reserved key `recordNumber`, and `queryFields(fields)` prepends it wherever a _query_
is built — the codec, the query schema, `buildRecordWhere`/`buildRecordOrderBy`, `DynamicTable`
and `RecordsFilterPanel`. Never where record _data_ is read or written (`buildRecordSchema`,
`DynamicForm`, the record cells), since it is not part of that data. Because it looks like a
TEXT field, the filter control, the URL format, the badge count and "Clear all" all work on it
unchanged; only the SQL projection knows better. `RECORD_COLUMN_SQL` in
`server/services/record-query.ts` is that one exception, and it declares both halves for the
same reason RELATION does: the number **filters as text** (`4` matches `#4`, `#14`, `#42`) but
**orders as an integer** (`#9` before `#10`).

`RESERVED_FIELD_KEYS` names the keys that belong to a record's own columns rather than a
table's fields (`createdAt`, `recordNumber`). `slugify` only emits `^[a-z0-9_]+$`, so a
camelCase key is already unreachable — seeding `uniqueKey`'s taken set with them just stops
that being true by luck.
...
- **Reuse before you paste.** A declaration block that would be the second copy of an existing one belongs in `_mixins.scss` instead. Check the mixin list there first — `stack`, `field-label`, `field-error`, `form-control`, `error-banner`, `page-header`, `page-title`, `page-empty`, `back-link`, `text-link` already cover most of what a new component or page needs.
- **Global Styles:** Split into multiple focused partials inside `app/assets/scss/`:
  - `_variables.scss` — CSS custom properties (colors, incl. the `*-tint` alpha washes)
...
- **Current State:**
  ...
  - **app/assets/scss/** — `main.scss` entry `@use`s `_variables.scss` (CSS custom props, incl. `--color-primary-tint` / `--color-danger-tint` — alpha washes spelled out because a custom property's alpha cannot be modified in CSS), `_reset.scss`, `_auth-form.scss`. `_functions.scss` (`rem()`) **and `_mixins.scss`** are both auto-injected into SFC styles via Vite `additionalData`; a standalone partial gets neither, so `_auth-form.scss` `@use`s both itself (and `_mixins.scss` `@use`s `functions`, since it does not re-export `rem()`). Mixins: `stack($gap)`, `field-label`, `field-error`, `form-control`, `error-banner`, `page-header`, `page-title`, `page-empty`, `back-link`, `text-link` — each replaced 2–6 copy-pasted blocks
  - **app/components/common/**
    ...
    - `BaseButton.vue` — button atom (type/disabled props, slot content); `primary` (default) / `danger` / `icon` / `ghost` / `link` variants, applied as one `base-button--${variant}` class. `icon` is a borderless icon-only button taking `icon` (iconify name) + `label` (aria-label/title); `link` is a bare text button for row actions (the chrome of a link, the semantics of a button) used for Rename/Edit/Delete on the dashboard and field manager. Both read a `--hover-color` custom property that each variant defaults for itself (`icon` → text, `link` → primary); the optional `hoverColor` prop overrides it via an inline style, which is how the Delete actions and the choice-remove button go danger-red. Per-variant defaults are why this is a custom property rather than the `v-bind` it used to be. The `ghost` variant is a transparent text+icon button with a faint indigo hover background (`--color-primary-tint`). First `@nuxt/icon` `<Icon>` usage in the app.
  ...
  - **shared/constants/** — the runtime registries, values only
    ...
    - `filter.ts` — `FILTER_VALUE_BY_TYPE` (per type: its `shape` and its `empty` value; the shape is the single fact that names its params _and_ tells the server how many bounds to compare), `DEFAULT_SORT_KEY` (`createdAt`) + `DEFAULT_SORT_DIR` (`desc` — **newest first**, so a record added now is at the top of page 1; the same `@@index([tableId, createdAt])` serves it, since Postgres scans a btree backwards), `RESERVED_QUERY_PARAMS` (`page`/`pageSize`/`sort`/`dir` — filter params share their namespace), `RECORD_NUMBER_KEY` + `RESERVED_FIELD_KEYS` (the keys naming a record's own columns rather than a table's fields — see "Record identity")
