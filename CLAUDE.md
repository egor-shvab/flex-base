...
- **Current State:**
  ...
  - **app/components/fields/** — the field-type→component registry (see "Adding a new field type")
    ...
    - `types.ts` — `IFieldInputProps` (`id`/`field`/`error`/`label` + `v-model` of `TRecordValue`), `IFieldCellProps` (`field`/`value`) and `IFieldFilterProps` (`id`/`field` + `v-model` of the type's `IFilterValueByType` value) — the contracts every entry honours
    ...
    - `{Text,Number,Boolean,Date,Select}FieldFilter.vue` — one filter control per type, each a `Base*` atom behind a **precisely typed `v-model`** and nothing else: TEXT/SELECT a `string` (`''` = not filtered; TEXT matching is always case-insensitive and partial), BOOLEAN a `boolean | null` (`null` = "All", since a two-state control cannot express "either"), NUMBER/DATE an `INumberRange` / `IDateRange`. **No filter component knows an operator** — the value is the whole contract, and `FILTER_VALUE_BY_TYPE` maps it to conditions at the serialization boundary. Free-text/number/date controls write through `useDebouncedModel` (300 ms); selects write at once
  ...
  - **app/components/table/**
    ...
    - `RecordsFilterPanel.vue` — the filter drawer (`BaseModal variant="drawer"`): one control per field, rendered generically from `FIELD_COMPONENTS[type].filter` and bound to `filters[field.key]` (falling back to the type's empty value), so it never learns which types exist. Merges a field's new value into the map, rebuilt in field order so URLs stay stable and dropping anything `isFilterValueEmpty`, so the map only ever holds active filters; emits `update:filters`. Footer shows the live match count (the drawer covers the table, so the count is the feedback) and a "Clear all" button
  ...
  - **app/composables/**
    ...
    - `useDebouncedModel.ts` — wraps a `v-model` in a responsive local `draft` plus a debounced write (default 300 ms), re-synced by a `watch` when the model changes from outside (clear all, a shared URL, the back button). Optional `normalize` runs on the way to the model only — `TextFieldFilter` trims with it, which trimming the draft would break mid-typing. Used by the TEXT/NUMBER/DATE filters, where every write costs a request
  - **app/layouts/**
