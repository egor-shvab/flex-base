...
4. One input component, **one cell component and one filter component** in `app/components/fields/`, plus one entry in `FIELD_COMPONENTS` (`app/components/fields/registry.ts`).
5. One entry in `FILTER_OPERATORS_BY_TYPE` (`shared/types/filter.ts`) listing the operators its filter may emit, and one in `VALUE_EXPR_BY_TYPE` (`server/services/record-query.ts`) projecting it to a comparable SQL expression.

Every one of these registries is typed as a total `Record<TFieldType, …>`, so adding an enum member is a compile error until its schema branch, components, operators and SQL projection exist.
...
- **Current State:**
  ...
  - **app/components/fields/** — the field-type→component registry (see "Adding a new field type")
    - `registry.ts` — `FIELD_COMPONENTS: Record<TFieldType, { input, cell, filter }>`, components `markRaw`ped; the only place `DynamicForm`/`DynamicTable`/`RecordsFilterPanel` learn about field types
    - `types.ts` — `IFieldInputProps` (`id`/`field`/`error`/`label` + `v-model` of `TRecordValue`), `IFieldCellProps` (`field`/`value`) and `IFieldFilterProps` (`id`/`field`/`conditions`) — the contracts every entry honours
    ...
    - `{Text,Number,Boolean,Date,Select}FieldFilter.vue` — one filter control per type, following `IFieldFilterProps` (`id`/`field`/`conditions`) + an `update:conditions` emit. Each owns the translation between its controls and its field's conditions: TEXT → one `contains` (a plain input; matching is always case-insensitive and partial), BOOLEAN/SELECT → one `eq` or none for "All", NUMBER/DATE → `gte`/`lte`. Free-text/number/date inputs debounce 300 ms; selects emit at once. NUMBER and DATE are thin wrappers over the shared `RangeFieldFilter.vue`, which holds **only** the filter wiring — conditions↔bounds mapping plus the debounce — and renders `BaseNumberRange` or `BaseDateRange` for all presentation. A condition's `TRecordValue` is narrowed (not cast) to each atom's bound type, so a value of the wrong shape reads as "no bound"
  ...
  - **app/components/table/**
    ...
    - `RecordsFilterPanel.vue` — the filter drawer (`BaseModal variant="drawer"`): one control per field, rendered generically from `FIELD_COMPONENTS[type].filter`, so it never learns which types exist. Merges a field's emitted slice into the full condition list, rebuilt in field order so URLs stay stable; emits `update:filters`. Footer shows the live match count (the drawer covers the table, so the count is the feedback) and a "Clear all" button
  ...
  - **app/utils/**
    - `debounce.ts` — `debounce(callback, delay = 300)`, used for the filter toolbar's value input
  ...
  - **server/api/**
    ...
    - `tables/[tableId]/records/` — `index.get` (paginated, sorted and filtered via `buildRecordQuerySchema(fields)`, gated by `requireOwnedTableFields`), `index.post`, `[recordId].patch`, `[recordId].delete`. Writes are gated by `requireRecordFields` and validated with `buildRecordSchema(fields)`, so the payload contract is the table's own metadata
  - **server/middleware/**
  ...
  - **server/services/**
    ...
    - `record-query.ts` — the only SQL in the project: `buildRecordWhere(tableId, fields, filter)` + `buildRecordOrderBy(fields, sort)`. Raw because Prisma cannot `orderBy` a JSON path. Two total maps hold the branching — `VALUE_EXPR_BY_TYPE` (per field type: `numeric`/`boolean` casts, plain text for TEXT/DATE/SELECT) and `OPERATOR_SQL` (per operator: `eq`, `contains` as `ILIKE` with escaped wildcards, and inclusive `gte`/`lte` range bounds). Conditions are ANDed, so a field contributing two of them needs no special case. Keys and values are bound as parameters, never interpolated, and the key is `::text`-cast to disambiguate Postgres' `->>` overloads
  - **server/utils/**
  ...
  - **shared/types/**
    ...
    - `record.ts` — `TRecordValue` / `TRecordData`, `IRecord`, `IRecordPage`, `IRecordQuery` (what the API resolved a request to: page + sort + `filters`) / `IRecordQueryState` (what the client holds and passes around)
    - `filter.ts` — `TFilterOperator` / `FILTER_OPERATORS` (`eq`, `contains`, `gte`, `lte`), `FILTER_OPERATORS_BY_TYPE` (per-field-type operator registry, total over `TFieldType`; each filter control implies its own operator, so this drives server validation and, via `PARAM_SUFFIX_BY_OPERATOR`, the params a field claims — **invariant:** no two operators of one type may share a suffix), `filterParamName(key, op)` / `filterParamNames(key, type)`, `RESERVED_QUERY_PARAMS` (`page`/`pageSize`/`sort`/`dir` — filter params share their namespace now), `IRecordFilter` (one condition; a field may contribute several and all are ANDed), `IRecordSort` / `TSortDirection` / `DEFAULT_SORT_KEY` (`createdAt`)
  - **shared/validation/**
    ...
    - `record.ts` — `VALUE_SCHEMA_BY_TYPE` (per-type `base` schema + `blank` value + `fromQuery` decoder, since a URL carries only strings), `buildRecordSchema(fields)` (builds a table's schema from its metadata; strips unknown keys), `blankValueFor(field)`, `buildRecordQuerySchema(fields)` (page + pageSize [default 50, hard cap 100] + sort/dir + the filter params the table's fields claim, validated against those fields and transformed into `IRecordQuery`; a **loose** object so the refinement can read the filter params without widening the base ones), `parseRecordFilters(fields, query)` / `toFilterParams` / `toRecordQueryParams` (the URL codec, shared by the schema, the page and the store, so a shared link and the fetch behind it cannot diverge). `claimFilterParams` resolves each param name to at most one field — reserved names first, then fields in order — so a legacy key that shadows another field's range bound stays deterministic. Required is enforced only where `blank` is `null`, so a BOOLEAN's `false` counts as a value

  **Filter wire format:** plain query params named after the field, with the operator implied by the name — `?company=acme&stage=Won&active=true&contract_value_from=1000&contract_value_to=5000&signed_on_from=2026-01-01`. TEXT (`contains`), SELECT/BOOLEAN (`eq`) use the bare key; NUMBER/DATE ranges use the `_from` (`gte`) / `_to` (`lte`) suffixes, from `PARAM_SUFFIX_BY_OPERATOR`. Every condition is ANDed. One value per param — a repeated param is a 400. An empty value (`?company=`) means "not filtered", never `ILIKE '%%'`. Params the table does not own are **ignored, not rejected**: with bare names a typo is indistinguishable from `utm_source`, so a stray param must not break the page (a malformed _known_ param — `?contract_value_from=abc` — is still a 400)
