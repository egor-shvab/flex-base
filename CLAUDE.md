...
- **Self-documenting names** (`fetchProductDetails`, not `getData`), and **the longer name wins wherever it removes a question** — `readCellValue` over `cellValue`, `adjustCachedCount` over `bumpCount`. A name states what a value **is**, not the shape it happens to have: `queryState` holds state, and `queryParams` would be the flat strings it serializes to. Where the code's own prose already names a concept — "a linked record", "a field's configuration" — that phrase is the name. Comment the _why_, not the _what_; use TSDoc for non-obvious business logic.
