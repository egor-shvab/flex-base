...
Covered today — `unit`: all of `shared/utils/`, all of `shared/validation/`, the SQL builder, field-key derivation, the JWT half of `server/utils/auth.ts`, the Prisma→HTTP error mapping, the four Vue-only composables, and all of `app/utils/` bar `record-cells.ts` plus `app/field-types/filter-summaries.ts` — those five at 100% including branches. `nuxt`: `useDetailLink`, `usePopover`, `useAnchoredPosition`, the tables and relations stores, `BaseButton`, and `RecordsFilterSummary`.

Uncovered and worth taking next: `BaseSelect` and `BaseModal`, the two components carrying real keyboard and focus contracts; then `useRecordDetail`, whose `useAsyncData` has caching semantics of its own; then the fields and records stores. `app/utils/record-cells.ts` and the `field-types` registries that import `.vue` are pure functions behind a component import, so they belong to the `nuxt` project despite having nothing to render.
