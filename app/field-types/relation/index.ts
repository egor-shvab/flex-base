 ...
 * to `false`, because `BaseSelect` ties the prop to its model's type.
 *
 * The candidates come from the target table, so this control fetches rather than reading the
 * field's metadata — the only entry whose component is not a `Base*` atom.
 */
...
export const RELATION_APP_FIELD_TYPE: IAppFieldType<'RELATION'> = {
  ...
  // The same picker the form uses, so a filter offers what a record can link to — but its
  // model is the target's **number**, which is what a filter puts in the URL. The server
  // resolves it back to an id before the SQL sees it (`docs/decisions.md`).
  filter: {
    component: markRaw(RelationFieldSelect),
    props: (field) => relationProps(field, 'All', false, 'number'),
  },
  // A multi-value field can only be asked whether it holds *any* of the filtered values
  multiFilter: {
    component: markRaw(RelationFieldSelect),
    props: (field) => relationProps(field, 'All', true, 'number'),
  },
  cell: markRaw(RelationFieldCell),
  // The only filter summary needing state beyond its value. An id outside the capped candidate
  // list resolves to nothing, and degrades the same way a cell does.
