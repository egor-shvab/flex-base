...
The focus ring was left at `3px` width / `2px` offset. An outline paints outside the border box, so its geometry is independent of the control height, and the three places where a 5px halo already crosses into a neighbour (`DynamicTable`'s actions `gap: rem(4)` and its action-cell inset, `AppSidebar`'s `gap: rem(2)`) were true at 44 and are unchanged — more conspicuous against a smaller box, not newly broken.
