...
| **The Vue layer is thinly tested**                                                                                                                                   | The blocker is gone: the suite is two projects, and `@nuxt/test-utils` boots the real app for anything needing `#imports`, a DOM or an SFC. Covered so far are the four Vue-only composables, both DOM composables, `useDetailLink`, the tables and relations stores, `BaseButton` and `RecordsFilterSummary` — enough to prove each seam (`registerEndpoint`, `mockNuxtImport`, `mountSuspended`) rather than to cover the layer. What is left is breadth: 21 of 23 components, three of five stores, and the pure helpers under `app/utils/` plus `app/field-types/filter-summaries.ts`, which need no Nuxt at all and are the cheapest of the lot                                         | **Open** — infrastructure done, breadth outstanding                                       |
