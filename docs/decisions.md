...
| **The Vue layer is thinly tested**                                                                                                                                   | The blocker is gone: the suite is two projects, and `@nuxt/test-utils` boots the real app for anything needing `#imports`, a DOM or an SFC. Covered are the four Vue-only composables, both DOM composables, `useDetailLink`, the tables and relations stores, `BaseButton` and `RecordsFilterSummary` — enough to prove each seam (`registerEndpoint`, `mockNuxtImport`, `mountSuspended`) — plus every pure helper under `app/utils/` and `filter-summaries.ts`, all at 100% branches. What is left is breadth, and it is now the expensive half: 21 of 23 components, three of five stores, `useRecordDetail`, and the `.vue`-importing registries                                        | **Open** — infrastructure done, breadth outstanding                                       |
