...
| **`npm run preview` cannot start on Windows**                                                                                                                        | `.output/server/index.mjs` assigns `globalThis._importMeta_` in its module **body**, but ESM hoists imports — so the chunk carrying the bundled Prisma client evaluates first, finds it unset, and falls back to the placeholder `file:///_entry.js`. Prisma then shims `__dirname` through `fileURLToPath()` on it: harmless on POSIX (`/_entry.js`), fatal on Windows, where a relative file URL throws `ERR_INVALID_FILE_URL_PATH`. Deployment targets are POSIX, so this is a local-tooling break rather than a production one. `test/e2e/setup/serve.mjs` works around it by assigning `_importMeta_` before a **dynamic** import, which is not hoisted                                 | **Open** — a two-line launcher would fix `preview` the same way                           |
| **A refused table delete gives the user no reason**                                                                                                                  | Deleting a table a relation still points at is correctly refused with a 409 naming the field to remove first — the guard holds and the data survives, which `test/e2e/relations.spec.ts` pins. But `ConfirmModal` renders no error and `useDeleteConfirm.confirm()` re-throws, so the message never reaches the screen: the dialog simply stays open. Found by writing the end-to-end spec for the refusal                                                                                                                                                                                                                                                                                   | **Open** — the copy exists on the server; nothing surfaces it                             |
