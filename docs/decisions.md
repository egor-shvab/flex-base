...
### Vitest runs on a plain Vite config, not `@nuxt/test-utils`

The unit suite covers `shared/utils/`, `shared/validation/` and the SQL builder — none of which touch Vue, the DOM or Nuxt's runtime. `defineVitestConfig` with `environment: 'nuxt'` would boot a Nuxt app instance per test file to serve code that imports nothing from it, and would drag the SCSS `additionalData` pipeline in behind it. `vitest.config.ts` instead restates the four aliases (`~~`, `#shared`, `#server`, `~`) that `.nuxt/tsconfig.*.json` already declares, so a spec resolves at runtime exactly as `vue-tsc` resolved it.

The cost is that the alias list is duplicated and can drift from the generated tsconfigs. That is cheap to notice — a drifted alias fails every spec that uses it immediately — and it is the only duplication in the arrangement. Add `@nuxt/test-utils` when the first component or composable test genuinely needs the runtime, not before.
...
| **No end-to-end tests**                                                                                                                                              | Vitest shipped: the URL codec, the filter predicates, the zod schemas and the SQL builder are covered, and CI gates on them. What is still unproven is everything only a browser can answer — a filtered URL rendering filtered on first paint, focus restoration, the detail chain under Back/Forward — which is the whole of `docs/architecture.md` §12. Playwright over the auth-gated pages is the remaining piece                                                                                                                                                                                                                                                                       | **Open** — next testing task (`CLAUDE.md` §10)                                            |
...
| **The Vue layer is untested**                                                                                                                                        | The unit suite is `environment: 'node'` with no DOM, so components, composables and stores have no coverage at all — `useForm`, `useDebouncedModel` and the field-type control registries are pure enough to test but need `@vue/test-utils`, which is deliberately not installed yet. The pure helpers under `app/utils/` and `app/field-types/filter-summaries.ts` need nothing new and are the cheap next step                                                                                                                                                                                                                                                                            | **Open** — start with the zero-DOM helpers                                                |
