...
- [x] 5. Opportunistic, one module at a time: the `useListboxNavigation` / `useSelectOptions` / `useAnchoredPosition` internals, `queryFields` → `queryColumns`, `bumpCount` → `adjustCachedCount`, `storeRow` → `cachedTableRow`, `IRecordSort.dir` → `direction` (the `?dir=` param keeps its spelling), `fieldSchema` / `tableSchema` / `credentialsSchema` → `*InputSchema`. `expr` → `expression` is deferred, not scheduled.
