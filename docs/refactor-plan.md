...
Small, independent extractions. Each removes a second copy of something the codebase already treats
as one decision.

### 2.1 — One debounce constant

- _Problem:_ 300 ms is declared three times and defaulted a fourth — `SEARCH_DEBOUNCE_MS`
  (`app/composables/useSelectOptions.ts:7`), `FILTER_DEBOUNCE_MS` (`app/field-types/filters.ts:14`),
  `SEARCH_DEBOUNCE_MS` (`app/pages/tables/[tableId]/index.vue:194`), and `delay = 300` in
  `useDebouncedModel`. Three of the four call it "the house figure" in prose.
- _Change:_ one exported constant — `app/utils/…` beside `shouldSearch`, which is the precedent for
  a house threshold living at the call sites' shared home. `useDebouncedModel`'s default reads it too.
- _Why:_ `CLAUDE.md` §6's DRY trigger fired three copies ago; a fourth surface with a different hold
  would be silently inconsistent.
- _Risk:_ **low.**

### 2.2 — One scalar→list normaliser

- _Problem:_ `toList` (`app/field-types/inputs.ts:42`) and `toCellValueList`
  (`app/utils/record-cells.ts:43`) are byte-identical implementations, carrying near-identical
  docblocks about the same pre-migration case. `collectRelationTargets`
  (`server/services/relations.ts:55`) does the same job inline on the server.
- _Change:_ one helper in `shared/utils/` (it is framework-free and both layers need it), read by
  `inputs.ts` and `record-cells.ts`. **Leave `collectRelationTargets` alone** — it normalises inside
  a loop that also filters non-strings, and folding it in would cost an allocation per row.
- _Why:_ the widening migration's tolerance is one rule; two copies can be relaxed apart.
- _Risk:_ **low.** Both sites are covered by existing specs.

### 2.3 — One route-param reader in the API layer

- _Problem:_ `const x = getRouterParam(event, 'x') ?? ''` appears 18 times across 13 handlers under
  `server/api/`. The `?? ''` is load-bearing (an absent param must not reach a query as `undefined`)
  and is restated everywhere.
- _Change:_ `routeParam(event, name): string` in `server/utils/`, used by every handler.
- _Why:_ handlers are meant to be thin; this is the one line in them that is not.
- _Risk:_ **low.** Covered end-to-end by the integration suite.

### 2.4 — One filter-map rebuild

- _Problem:_ `RecordsFilterPanel.applyFieldValue` and `RecordsFilterSummary.remove` are the same
  algorithm — walk the columns in field order, substitute one field's value, drop everything
  `isFilterValueEmpty`. `RecordsFilterSummary`'s own comment says so ("the same rebuild the drawer
  does"). Both components additionally recompute `filterableFields(queryColumns(props.fields))`
  identically.
- _Change:_ `withFilterValue(columns, filters, key, value): TRecordFilterValues` in
  `shared/utils/filter.ts`, beside the predicates it uses. Both components call it — the summary
  passing `emptyFilterValueFor(field)` as the value, which is what its "remove" already means.
- _Why:_ the field-order rebuild is what keeps a shared URL stable whichever control was touched.
  Two copies is two places for that property to be lost.
- _Risk:_ **low–medium** — both are pinned by `*.nuxt.spec.ts` files.

### 2.5 — Deduplicate the registry props factories

- _Problem:_ `app/field-types/inputs.ts` and `filters.ts` carry four near-identical `props(field)`
  factories: `FIELD_INPUTS.SELECT` vs `MULTI_INPUTS.SELECT` (differ by `multiple`),
  `FIELD_INPUTS.RELATION` vs `MULTI_INPUTS.RELATION` (same), and `FIELD_FILTERS.RELATION` vs
  `MULTI_FILTERS.RELATION` (same). Each SELECT factory also calls `choiceOptions(field)` **twice** —
  once for `options`, once for `.length` — allocating the array twice per render.
- _Change:_ two local factories, `selectProps(field, { multiple, placeholder })` and
  `relationProps(field, { multiple, placeholder })`, each computing `choiceOptions` once. The
  override tables keep their entries and their totality; only the object bodies collapse.
- _Why:_ the `MULTI_*` tables exist so cardinality is declared per type. That contract is about
  _which_ entry exists, not about restating nine attributes to add one.
- _Risk:_ **low.** `inputs.nuxt.spec.ts` and `filters.nuxt.spec.ts` pin the emitted props.

### 2.6 — One "N matching records" phrase, and one "Delete table" confirmation

- _Problem:_ two user-visible strings exist twice, spelled differently in one case.
  `RecordsFilterSummary.countLabel` builds `1 matching record` / `N matching records`;
  `RecordsFilterPanel`'s footer builds the same sentence inline with different pluralisation code.
  The "Delete <b>name</b>? All of its fields and records will be permanently removed…" body is
  duplicated verbatim in `app/pages/index.vue:52` and `app/pages/tables/[tableId]/settings.vue:154`.
- _Change:_ one formatter for the count (an `app/utils/` helper, or the filter summary exporting it);
  for the dialog, decide between a shared constant and accepting the duplication — the two dialogs
  are otherwise different components. The count is the one worth doing.
- _Why:_ the e2e suite selects by accessible name, so a divergence here is a broken spec, and the
  copy is the same claim in both places.
- _Risk:_ **low.**
