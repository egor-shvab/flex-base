...
**Cells.** `{text,number,boolean,date,select,relation}/*FieldCell.vue` — one read-only cell per type, beside the module that names it. BOOLEAN renders an `mdi:check`/`mdi:minus` icon, SELECT a chip, NUMBER/DATE fixed `en-GB` `Intl` formats, RELATION the label the page resolved (or a muted "Unknown record"). Blank values never reach a cell — `RecordsTable` renders the `—` placeholder itself. `RecordNumberCell` and `TimestampCell` belong to `RECORD_COLUMNS` rather than to a field type, which is why they stay in `cells/` and are not named `*FieldCell`.
