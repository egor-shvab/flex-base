...
- **`RECORD_COLUMNS`** (`app/field-types/record-columns.ts`) — where the value comes from (`record.number` / `record.createdAt` / `record.updatedAt`, never `record.data`) and which cell renders it. `RecordsTable` consults it first and falls through to `FIELD_CELLS`, so it never learns which columns exist.
