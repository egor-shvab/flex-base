...
- **`address.ts` is the one place a URL segment becomes a row's number.** `parseAddressNumber`
  ...
  malformed address 404s rather than reaching Prisma as `NaN`; `parseTableAddress` additionally
  ignores a `-…` suffix. Both sides read it, so the bound cannot drift between the page and the
  server.
...
| ------------- | ------------------------- | ---------------------- |
| `registry.ts` | `inputFor(field)`         | `RecordForm`           |
...
- `TRecordFieldControl` = `Required<IFieldControl<TRecordValue>>`, since a record input always adapts (a DOM control speaks strings and checkboxes, never `TRecordValue`) — which is why `RecordForm` never branches on an optional adapter.
- `TFilterSummary` + `IFilterSummaryContext` — a summariser and the one thing it may need beyond its value (only RELATION uses it, to resolve a linked record). `TFieldConfigSummary` + `IFieldConfigSummaryContext` are their metadata-side twins: no value at all, and the context carries a `tableName` lookup for the one type whose configuration names another table.
...
Three fragment modules keep the per-type modules to their own wording — **`adapters.ts`** (the control-to-value shims), **`prose.ts`** (how a filter value reads), and the server's **`fragments.ts`** (the projections, comparators, search predicates and ordering pieces the type modules are assembled from). The dividing line is what varies **per type**: anything a field type composes lives in a fragment module, and `record-sql.ts` keeps only what does not — `RECORD_COLUMN_SQL` and the `buildRecord*` builders.
...
**Cells.** `{text,number,boolean,date,select,relation}/*FieldCell.vue` — one read-only cell per type, beside the module that names it. BOOLEAN renders the words `Yes`/`No` — a tick beside nothing is ambiguous, and `false` is a value rather than a blank — SELECT a chip, NUMBER/DATE fixed `en-GB` `Intl` formats, RELATION the label the page resolved (or a muted "Unknown record"). Blank values never reach a cell: `RecordFieldValue` answers for them first. `RecordNumberCell` and `TimestampCell` belong to `RECORD_COLUMNS` rather than to a field type, which is why they stay in `cells/` and are not named `*FieldCell`.
...
`queryColumns(fields)` returns them around a table's own fields in presentation order — `Record # | …fields… | Created at | Updated at` — and is applied wherever a **query** is built: the codec, the query schema, `buildRecordWhere`/`buildRecordOrderBy`, `RecordsTable`, `RecordsFilterPanel`, `RecordsFilterSummary`. It is **never** applied where record **data** is read or written (`buildRecordSchema`, `RecordForm`, the field manager), since none of it is part of that data — which is what makes these columns generic and read-only.
...
- **`RECORD_COLUMNS`** (`app/field-types/cell-resolver.ts`) — where the value comes from (`record.number` / `record.createdAt` / `record.updatedAt`, never `record.data`) and which cell renders it. `RecordsTable` consults it first and falls through to `FIELD_CELLS`, so it never learns which columns exist.
...
- **`RelationService.resolveFilterTargets`** — runs before the list's WHERE is built and swaps each relation filter's _address_ for the id the column stores. One `findMany` per distinct target table. **Nothing is dropped**: a value it cannot resolve passes through unchanged, which is what stops an unmatched filter from vanishing and widening the list to the whole table (`decisions.md` → _A relation filter is resolved from numbers to ids before the SQL sees it_).
- **`RelationService.listRelationOptions(field, search)`** — backs `GET /api/tables/[tableAddress]/fields/[fieldId]/options[?q=]`, capped at `RELATION_OPTIONS_LIMIT` and label-ascending. Scoped by the **source field**, so nothing about the target is taken from the client. `?q=` narrows via `buildRecordLabelSearch` — the label field plus the record's `#number` — leaving the ORDER BY untouched, so search and order stay independent. No `SEARCH_MIN_LENGTH` here (`decisions.md` → _Relation option search deliberately does not enforce `SEARCH_MIN_LENGTH`_); the bound is `max(100)` on the term. Note the order is by **label**, so the visible numbers are not ascending and blank-labelled records sort together at the end.

Client side, `app/stores/relations.ts` is the single home for all of it — `optionsByField` (a picker's candidates) and `linkedByField` (id → `ILinkedRecord`), plus `linkedByFieldNumber`, the same records keyed the way a **filter** addresses them; all three keyed by field id. `RelationFieldSelect` reads the options, in whichever currency its `valueBy` names — a record input speaks ids, a filter speaks numbers. `RelationFieldCell` reads the linked records, and takes the target table's number from the **tables** store: a relation stores a cuid, and a link has to carry an address. A number it cannot resolve degrades to plain text, never a broken link. Both draw the pair through **`BaseLinkedRecord`**, the one component that writes a `#`; the picker reaches it through `BaseSelect`'s single slot (`decisions.md` → _`BaseSelect` has one slot, and it replaces an option's text rather than its row_), and each option's flat `label` stays `formatLinkedRecord`'s output so the trigger, the type-ahead and the accessible name all agree with the row.
...
**A search is the same query shape as any other list.** `listRecords` composes one statement whether or not a search is running; the ordering's join, when it brings one, lands in that statement's `FROM`. A search was once wrapped in a materialised CTE to force the search ahead of the ordering — measurement retired it, because the hint made cost track the match count rather than the page size (`decisions.md` → _A search is one query shape, and the planner is left to choose_).
...
- **`api/tables/[tableAddress]/records/index.get.ts`** — validates with `buildRecordQuerySchema(fields)`, then composes `IRecordQuery` from `parseRecordQueryState(fields, params)` + the validated `pageSize` — **the schema judges, the codec decodes**
...
- **`composables/useEntityFormModal.ts`** — the create-or-edit dialog every list page opens: whether it is showing, and which row on. **Two refs rather than a `{ mode } | null` union**, which is what keeps a generic payload name out of the templates — each page names its own `editingRecord` / `editingField` / `editingTable` and binds it straight to the dialog's prop, and the mode's spelling stays the caller's (the dashboard says `rename` where the others say `edit`). `openCreate` clears the last edited row, or a create opened after an edit would render pre-filled
...
- **`composables/useFieldControls.ts`** — each field's control resolved through a registry, for the two metadata renderers and nothing else. A registry entry's `props` is a **factory**, so resolving inside a `computed` is what builds each control's props once per change of the field list rather than per render. Generic on the control, which is what hands `RecordForm` its adapters back as **required** (`inputFor` answers `TRecordFieldControl`) while the drawer keeps optional ones — an identity default here would erase that difference
...
- **`RecordForm.vue`** — renders a form from `IField[]` by walking `inputFor`: `v-bind`s each entry's `props(field)`, passes the value through `toControl`, pushes what the control emits back through `fromControl`. Values flow down as props and changes back up via `update: [key, value]`, so **the parent's `useForm` object is never mutated**.
- **`RecordsTable.vue`** — renders from `queryColumns(fields)` + `IRecord[]`, so **one** `columns` list drives header and body alike. Each cell is a `RecordFieldValue`. No branch on a key or a type anywhere in the template. Emits `edit`/`delete`/`sort`; the optional `sort` prop drives `aria-sort` and the header arrow. The row's **View** action emits nothing — it is a `<NuxtLink>` through `useDetailLink`, which is why the component takes a `tableId` prop: a generic renderer must not read that off the route itself.
...
- **`TableFieldList.vue`** — the settings page's field rows: type, configuration and key per row, so a table's shape reads without opening a dialog per field. Nothing branches on a type — the word, the glyph and the configuration line all come from registries, the cardinality from `isMultiValue`. It takes the config-summary context as a prop rather than reading the tables store, so a RELATION's target name arrives from the page and the component stays props-and-emits. It emits `create`/`edit`/`delete`; the section heading, the count and the primary "Add field" stay on the page, because `.section-head` is shared with the Table section above it.
