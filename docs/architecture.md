...
Multi-value **cells** need no override table at all. `cellComponent` returns one shared `MultiValueCell`, which renders each entry through `FIELD_CELLS[field.type]` — a list of values is the list of how each value renders, so a future multi-capable type is covered without a component of its own. It renders **inline** rather than as a flex row, which is load-bearing: an `inline-flex` list is a single atomic box that `text-overflow` cannot reach inside, so an over-full one was hard-clipped with no ellipsis. Inline, the values that fit are drawn in full and the rest give way to one. Nothing about the cell puts it on a line — `DynamicTable`'s `white-space: nowrap` does that, and `RecordDetail` simply does not impose it.
...
| `utils/field-key.ts`                        | `slugify` (a display name → `^[a-z0-9_]+$`, `field` when nothing survives) + `buildFieldKey(name, type, existing)` — the immutable key a new field takes. A key must be free for **every query param it would claim**, not only for itself: a field called "Page" becomes `page_2`, "Budget from" becomes `budget_from_2` next to a NUMBER `budget`, since that field already claims both bounds. Server-only — nothing in the Vue layer derives a key                                                                                                                                                                                            |
...
| `services/fields.ts`                        | list/create/update/delete scoped by `tableId`; derives `order` and the DB `options`, and takes the immutable `key` from `utils/field-key.ts`. Rejects type changes, RELATION retargeting, and narrowing a multi-value field back to a single one (400 each); **widening** a field to multi-value is allowed and runs `widenToList` — one scoped, idempotent `UPDATE` wrapping each stored scalar in an array — inside the same transaction as the metadata change, so the rows and the schema that describes them move together. Exports `fieldSelect` + `toFieldMetadata`, the one place Prisma's untyped `options` JSON is narrowed to `IField` |
...
| `stores/tables.ts`                    | `loaded`/`failed` flags + `ensureTables()`, which **never throws** — it sets `failed` and the sidebar reports it inline with a Retry. Also owns `bumpCount(tableId, key, delta)`, the one way the cached `_count` moves without a refetch: **`stores/records.ts` and `stores/fields.ts` are its only callers**, on a create or a delete. No page refetches the list to correct a count                                                                   |
...
- **`RecordDetail.vue`** — the detail dialog's body: a `<dl>` over `queryFields(fields)`, the same list `DynamicTable` renders, minus the record number (which names the dialog in its own heading). Values wrap instead of truncating — reading one in full is the point of the dialog. It does that by **not** declaring the table's `white-space: nowrap`, not by overriding a cell; the only thing it adds is the `line-height` that spaces a wrapped multi-value cell's rows, since inline content has no `row-gap`.
...
Four lines are **approximated rather than proven**, and their specs say so where they sit:
...
- **a multi-value cell ending in an ellipsis** — the entry that does not fit keeps its box, its client rects and its `checkVisibility()`; only the painting changes, so nothing in the DOM distinguishes it from the hard clip it replaced. What is asserted instead is the half that _is_ structural: the values that fit are drawn in full, which is what the obvious wrong fix would break.
...
- **Accessibility:** no `serious` or `critical` axe violation on the dashboard, table settings, the records list, the record form or the filter drawer — and none on the dashboard or records list at 375px, where the shell is a different layout.
...
- **The mobile shell** (375×812): the sidebar is `visibility: hidden` until opened, so nothing inside it is reachable; the toggle tracks `aria-expanded`; navigating or clicking the scrim dismisses it; the records table scrolls inside its own container rather than making the page scroll sideways; a dialog fits the viewport.
