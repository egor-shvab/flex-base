...
`cellComponent` is paired with **`toCellValueList`** / **`toCellSingleValue`** (`app/utils/record-cells.ts`): whenever the first returns `MultiValueCell`, the value is `toCellValueList`, otherwise it is `toCellSingleValue`. `RecordFieldValue` branches on the same `isMultiValue` question to pick the pair. That is what lets each cell declare the exact shape it renders instead of the union of both — and `toCellValueList` is **the one place** a stored value that is not yet an array is accounted for (a row drawn before `updateField`'s migration ran).
