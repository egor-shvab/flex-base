...
describe('record numbers', () => {
  it('starts at one', async () => {
    const record = await createRecord(tableId, fields, { company: 'Acme' })

    expect(record.number).toBe(1)
  })

  it('increments per record', async () => {
    await createRecord(tableId, fields, { company: 'Acme' })
    await createRecord(tableId, fields, { company: 'Beta' })
    const third = await createRecord(tableId, fields, { company: 'Gamma' })
  ...
  it('gives every concurrent create a distinct number', async () => {
    const created = await Promise.all(
      Array.from({ length: 20 }, (_, index) =>
        createRecord(tableId, fields, { company: `Co ${index}` }),
      ),
    )
  ...
  it('never reuses the number of a deleted record', async () => {
    const first = await createRecord(tableId, fields, { company: 'Acme' })
    await createRecord(tableId, fields, { company: 'Beta' })

    await deleteRecord(tableId, first.id)
    const next = await createRecord(tableId, fields, { company: 'Gamma' })
  ...
  it('counts per table rather than globally', async () => {
    ...
    await createRecord(tableId, fields, { company: 'Acme' })
    const elsewhere = await createRecord(other.id, otherFields, { company: 'Beta' })
...
describe('writing a record', () => {
  it('round-trips the data through JSONB', async () => {
    const stored = await createRecord(tableId, fields, { company: 'Acme' })
  ...
  it('replaces the data wholesale rather than merging', async () => {
    ...
    await updateRecord(tableId, fields, record.id, { company: 'Beta' })
    const page = await listRecords(tableId, fields, query())
  ...
  it('moves updatedAt but not createdAt', async () => {
    const record = await createRecord(tableId, fields, { company: 'Acme' })
    const updated = await updateRecord(tableId, fields, record.id, { company: 'Beta' })
  ...
  it('404s when updating a record of another table', async () => {
    ...
    await expect(
      updateRecord(other.id, fields, record.id, { company: 'Beta' }),
  ...
  it('404s when deleting a record of another table, leaving it in place', async () => {
    ...
    await expect(deleteRecord(other.id, record.id)).rejects.toMatchObject({ statusCode: 404 })
